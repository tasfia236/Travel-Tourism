...
async function run () {
  try {
    ...
    app.post('/request-guide', verifyToken, verifyUser, async (req, res) => {
      const { user } = req.body
      ...
      if (requestedUsers.find(u => u.email === user.email)) {
        return res
          .status(400)
          .json({ message: 'User has already requested to be a tour guide.' })
      }
    ...
    app.get('/tourtype/:type', async (req, res) => {
      ...
      const result = await spotCollection
        .find({ tour_type: req.params.type })
        .toArray()
      res.send(result)
      // console.log(result);
    })
    ...
    app.get('/wishlist/:email', verifyToken, verifyUser, async (req, res) => {
      const userEmail = req.params.email
      try {
        const wishlistItems = await spotCollection
          .find({ wish_email: userEmail })
          .toArray()
    ...
    app.patch('/wishspots/:id', async (req, res) => {
      ...
      try {
        ...
        await spotCollection.updateOne(filter, {
          $set: { wishlist: wishlistStatus }
        })
